...
async def get_channel_by_name(name: str) -> dict | None:
    async with aiosqlite.connect(str(DB_PATH)) as db:
        db.row_factory = aiosqlite.Row
...
async def send_to_channel(channel_name: str, title: str, text: str, at_user_ids: list[str] = None):
...
async def send_to_all_channels(title: str, text: str, at_user_ids: list[str] = None):
...
async def send_overdue_summary(overdue_items: list[dict], channel_name: str = None):
